	...
	if (Tab == SHIPS_DATA_TAB) {
		...
		for (int i=0; i<Current_list_size; i++) {
			if ((Current_list[i].model_num > -1) && (Current_list[i].textures_loaded)) {
				...
				ship_page_out_texture_set(&Ship_info[Current_list[i].index], true);
		...
		ship_init_texture_set(&Ship_info[Cur_entry_index], model_get(Techroom_ship_modelnum));
...
void techroom_ships_render(float frametime)
{
	...
	model_render(Techroom_ship_modelnum, &Techroom_ship_orient, &vmd_zero_vector, render_flags, -1, -1, sip->texture_set);
	...
	if (Ship_list != NULL) {
		for (i = 0; i < Ship_list_size; i++) {
			if (Ship_list[i].model_num >= 0) {
				...
				ship_page_out_texture_set(&Ship_info[Ship_list[i].index], true);
			}
		}
