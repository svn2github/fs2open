...
ship		*Player_ship;
texture_map	*Player_cockpit_textures;
...
void init_ship_entry(ship_info *sip)
{
	...
	sip->cockpit_pof_file[0] = '\0';
	vm_vec_zero(&sip->cockpit_offset);
	sip->pof_file[0] = '\0';
	...
	memset(&sip->collision_physics, 0, sizeof(ship_collision_physics));
	sip->collision_physics.both_small_bounce = 5.0;
	...
	memset(sip->restricted_loadout_flag, 0, sizeof(int) * MAX_SHIP_WEAPONS);
	memset(sip->allowed_bank_restricted_weapons, 0, sizeof(int) * MAX_SHIP_WEAPONS * MAX_WEAPON_TYPES);
	...
	memset(sip->nondark_colors, 0, sizeof(ubyte) * MAX_NONDARK_COLORS * 3);
	...
	generic_bitmap_init( &sip->thruster_distortion_info.normal );
	generic_bitmap_init( &sip->thruster_distortion_info.afterburn );
...
void ship_level_init()
{
	...
	for (i = 0; i < Num_ship_classes; i++)
		ship_page_out_texture_set(&Ship_info[i], true);
}
...
void ship_set(int ship_index, int objnum, int ship_type)
{
	...
	shipp->ship_replacement_textures = sip->texture_set;
}
...
void ship_page_out_texture_set(ship_info *sip, bool release)
{
	int i;
	...
	for (i = 0; i < MAX_MODEL_TEXTURES; i++) {
		texture_map *tmap = &sip->texture_set[i];

		if (tmap->set_flag != TMR_SET_SHIP) {
			continue;
		}

		tmap->PageOut(release);
	}
...
void ship_init_texture_set(ship_info *sip, polymodel *pm)
{
	int i, j;
...
void ship_page_in()
{
	...
	for (i = 0; i < MAX_SHIPS; i++)	{
		...
		if (sip->model_num >= 0) {
			...
			model_set_active_textures(sip->model_num, Ships[i].ship_replacement_textures);
