...
void model_render(int model_num, matrix *orient, vec3d * pos, uint flags, int objnum, int lighting_skip, texture_map *replacement_textures, const bool is_skybox)
{
	int cull = 0;
	...
	// set any alternate textures as active set
	if (replacement_textures != NULL)
		pm->maps = replacement_textures;
...
void submodel_render(int model_num, int submodel_num, matrix *orient, vec3d * pos, uint flags, int objnum, texture_map *replacement_textures)
{
	polymodel * pm;
	...
	// set any alternate textures to active set
	if (replacement_textures != NULL)
		pm->maps = replacement_textures;
...
void model_page_in_textures(int modelnum, int ship_info_index)
{
	...
	if (ship_info_index >= 0)
		ship_page_in_textures(&Ship_info[ship_info_index]);
}
...
void model_page_out_textures(int model_num, bool release)
{
	...
	for (i = 0; i < Num_ship_classes; i++) {
		if (Ship_info[i].model_num == model_num) {
			ship_page_out_texture_set(&Ship_info[i], release);
...
int texture_info::LoadTexture(char *filename, char *dbg_name = "<UNKNOWN>")
{
	...
	this->texture = bm_load_either(filename, NULL, NULL, NULL, 1, CF_TYPE_MAPS);
	if(this->texture < 0)
