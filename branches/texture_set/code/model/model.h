...
public:
	...
	texture_map()
		: is_ambient(false), is_transparent(false), set_flag(TMR_SET_ORIGINAL)
	{
...
// Renders a model and all its submodels.
// See MR_? defines for values for flags
void model_render(int model_num, matrix *orient, vec3d * pos, uint flags = MR_NORMAL, int objnum = -1, int lighting_skip = -1, texture_map *replacement_textures = NULL, const bool is_skybox = false);
