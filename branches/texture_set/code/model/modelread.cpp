...
void model_load_texture(texture_map *tmap, char *file, char *loader_name)
{
	...
	tmap->Clear();
	strcpy_s(tmap->filename, tmp_name);
	...
	else
	{
		...
		tbase->LoadTexture(tmp_name, loader_name);
	...
	else
	{
		...
		tglow->LoadTexture(tmp_name, loader_name);
	}
	// -------------------------------------------------------------------------
	...
	else
	{
		...
		tspec->LoadTexture(tmp_name, loader_name);
	}
	...
	if ( (!Cmdline_normal && !Fred_running) || (tbase->GetTexture() < 0) ) {
		...
		tnorm->LoadTexture(tmp_name, loader_name);
	}
	...
	if ((!Cmdline_height && !Fred_running) || (tbase->GetTexture() < 0)) {
		...
		theight->LoadTexture(tmp_name, loader_name);
	}
	...
	tmisc->LoadTexture(tmp_name, loader_name);
