	...
	if (change_model) {
		...
		if (Lab_model_num != -1) {
			...
			if (Lab_last_selected_ship >= 0) {
				ship_page_out_texture_set(&Ship_info[Lab_last_selected_ship], true);
		...
		if (model_fname != NULL) {
			...
			if (valid_ship) {
				if (Lab_model_num >= 0) {
					...
					ship_init_texture_set(&Ship_info[sel_index], model_get(Lab_model_num));
				}
...
void labviewer_render_model(float frametime)
{
	...
	if (sip != NULL) {
		// go ahead and set default texture set as well
		texture_set = sip->texture_set;
	...
	if (Lab_viewer_flags & LAB_FLAG_SHOW_DEBRIS) {
		...
		for (i = 0; i < pm->num_debris_objects; i++) {
			...
			submodel_render(Lab_model_num, pm->debris_objects[i], &Lab_viewer_orient, &world_point, Lab_model_flags, -1, texture_set);
		}
	}
	// render normally otherwise
		...
		model_render(Lab_model_num, &Lab_viewer_orient, &vmd_zero_vector, /*Lab_model_flags*/flagggs, -1, -1, texture_set);
	}
