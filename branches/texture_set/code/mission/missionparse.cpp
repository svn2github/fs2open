...
int parse_create_object_sub(p_object *p_objp)
{
	...
	if (p_objp->texture_set != NULL)
		shipp->ship_replacement_textures = p_objp->texture_set;
...
p_object::p_object()
	: next(NULL), prev(NULL), dock_list(NULL), created_object(NULL), texture_set(NULL)
{}
...
p_object::~p_object()
{
	...
	parse_object_page_out_texture_set(this, true);
}
...
int parse_object(mission *pm, int flag, p_object *p_objp)
{
	...
	if ( optional_string("$Texture Replace:") || optional_string("$Duplicate Model Texture Replace:") )
	{
		...
		while ( optional_string("+old:") )
		{
			...
			strcpy_s(trp.ship_name, p_objp->name);
			...
			// *** account for FRED
			if (Fred_running)
				Fred_texture_replacements.push_back(trp);
...
void parse_object_page_out_texture_set(p_object *p_objp, bool release)
{
	...
	for (i = 0; i < MAX_MODEL_TEXTURES; i++) {
		texture_map *tmap = &p_objp->texture_set[i];
