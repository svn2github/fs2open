...
BOOL CLauncherDlg::OnInitDialog()
{
	...
	char *exe_path = Settings::exe_filepath;

	// If no settings file exists use default settings
	if(Settings::get_exe_path_from_ini() == false)
	{
		const int MAX_FILENAME_TRYS = 3;
...
void CLauncherDlg::OnApply() 
{
	...
	if (ini) {
		iniparser_setstr(ini, "launcher:exe_filepath", Settings::exe_filepath);
	...
	if(Settings::exe_type == EXE_TYPE_CUSTOM || flags & FLAG_FS2)
	{
...
void CLauncherDlg::OnRun() 
{
	...
	bool result = run_file( 
		(LPTSTR) Settings::exe_nameonly, 
		(LPTSTR) Settings::exe_pathonly,
...
void CLauncherDlg::NewExeSet(char *exe_path)
{
		...
		if ( !last_slash || !stricmp(current_dir, just_path) ) {
			if ( !last_slash ) {
				...
				if ( file_exists(just_path) ) {
					Settings::set_exe_filepath(just_path);
					good_path = true;
				}
			} else {
				Settings::set_exe_filepath(exe_path);
				good_path = true;
			}
		}
	...
	GetDlgItem(IDC_RUN)->EnableWindow( Settings::exe_path_valid );
	GetDlgItem(IDC_PATH)->SetWindowText(Settings::exe_filepath);	

	tab_video.Update(Settings::exe_type, tab_comm_line.GetFlags());

	tab_comm_line.LoadSettings(Settings::reg_path);
